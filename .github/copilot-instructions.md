...
The marketplace currently supports skills-only plugins with bash helper scripts:
...
1. **Lint all** — `npm run lint:all` (markdown + ESLint, must all pass)
...
| --- | --- |
| `npm run lint` | Markdown files only (markdownlint) |
| `npm run lint:js` | JavaScript files (ESLint) |
| `npm run lint:all` | All of the above |
| `npm run lint:fix` | Auto-fix markdown + ESLint issues |
...
- Use `*.sh` extension for all scripts — bash is native on the Debian/ARM targets
